---
title: Accessing Historical Session Notes
section: finding-your-mentees-and-sessions
description: How to find, view, and edit the session notes recorded against each of your mentees.
media:
  src: assets/images/accessing-historical-session-notes/session-notes.webm
  alt: Opening a mentee's page from My Mentees to view their session notes
  header: Viewing a mentee's session notes
steps:
  - title: Overview
    body: |
      Each time you [wrap up a session](../wrap-up-a-session/), the session notes are saved against the mentee on the Community Site. This page shows you how to find those notes again later, so you can review them before your next session or edit any details that need updating.
  - title: Log In to the Community Site
    body: |
      First, log in to the [Community Site](https://mindjam.my.site.com/Portal/s/). If this is your first time logging in, follow the steps on the [First Time Logging In](../first-time-logging-in/) page.
  - title: Open My Mentees
    body: |
      Once you are logged in, go to **Appointment Management** and then select **My Mentees**. This opens the list of mentees currently assigned to you.
  - title: Open the Mentee's Page
    body: |
      In the **Assigned Mentee Name** column — the second column in the table — click on the mentee's name. The name is shown as a hyperlink. You will then be taken to the mentee's page, where you can view their session notes.
  - title: Edit a Note
    body: |
      To edit any of your notes, click the small **pencil icon** next to the relevant note. This will allow you to make changes to that note and save them.
  - title: How Notes Get Here
    body: |
      Each time you [wrap up a session](../wrap-up-a-session/), the session notes will be added to this section automatically. You do not need to copy or move them yourself — they will appear against the mentee as soon as the session is wrapped up.
---
